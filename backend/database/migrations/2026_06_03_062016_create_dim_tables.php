...
return new class extends Migration
{
    public function up(): void
    {
        $sql = <<<'SQL'
-- ============================================================
-- TABLAS DIMENSIONALES ARMORA NextGen
-- Catálogos SUNAT + Catálogos de Negocio
-- ============================================================
...
-- 5. dim_documento_simbolo (SUNAT Cat. 01)
CREATE TABLE IF NOT EXISTS dim_documento_simbolo (
    id              SERIAL PRIMARY KEY,
...
-- 6. dim_departamento (SUNAT Cat. 13 / INEI)
...
-- 7. dim_provincia (SUNAT Cat. 13 / INEI)
...
-- 8. dim_ubigeo (SUNAT Cat. 13 / INEI) - Lima Metropolitana
CREATE TABLE IF NOT EXISTS dim_ubigeo (
    id              SERIAL PRIMARY KEY,
...
-- 9. dim_tipo_afeccion_igv (SUNAT Cat. 07)
CREATE TABLE IF NOT EXISTS dim_tipo_afeccion_igv (
    id              SERIAL PRIMARY KEY,
    codigo_sunat    VARCHAR(2)  UNIQUE NOT NULL,
    nombre          VARCHAR(100) NOT NULL,
...
-- 10. dim_tipo_calculo_isc (SUNAT Cat. 08)
CREATE TABLE IF NOT EXISTS dim_tipo_calculo_isc (
    id              SERIAL PRIMARY KEY,
    codigo_sunat    VARCHAR(2)  UNIQUE NOT NULL,
    nombre          VARCHAR(100) NOT NULL,
    descripcion     TEXT,
...
-- 11. dim_nota_credito_tipo (SUNAT Cat. 09)
CREATE TABLE IF NOT EXISTS dim_nota_credito_tipo (
    id              SERIAL PRIMARY KEY,
    codigo_sunat    VARCHAR(2)  UNIQUE NOT NULL,
    nombre          VARCHAR(100) NOT NULL,
    activo          BOOLEAN DEFAULT true,
...
-- 12-22. Catálogos de negocio
CREATE TABLE IF NOT EXISTS dim_segmento_sunat (
    id SERIAL PRIMARY KEY, codigo VARCHAR(10) UNIQUE NOT NULL,
    nombre VARCHAR(100) NOT NULL, activo BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS dim_tipo_cliente (
...
INSERT INTO dim_tipo_cliente (nombre) VALUES
    ('Mayorista'),('Minorista'),('Corporativo'),
    ('Consumidor Final'),('Distribuidor'),('Gobierno')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_familia_sunat (
    id SERIAL PRIMARY KEY, codigo VARCHAR(10) UNIQUE NOT NULL,
    nombre VARCHAR(100) NOT NULL, activo BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
INSERT INTO dim_familia_sunat (codigo, nombre) VALUES
    ('ALIM','Alimentos y Bebidas'),('TEXT','Textiles'),
    ('QUIM','Químicos'),('FARM','Farmacéuticos'),
    ('MADR','Madera y Papel'),('MIN','Minería'),
    ('MET','Metales'),('MAQ','Maquinaria'),
    ('VEH','Vehículos'),('SERV','Servicios')
ON CONFLICT (codigo) DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_clase_sunat (
    id SERIAL PRIMARY KEY, codigo VARCHAR(10) UNIQUE NOT NULL,
    nombre VARCHAR(100) NOT NULL,
    familia_id INT REFERENCES dim_familia_sunat(id),
    activo BOOLEAN DEFAULT true
);
...
CREATE TABLE IF NOT EXISTS dim_producto_clase (
    id SERIAL PRIMARY KEY, nombre VARCHAR(100) NOT NULL,
    descripcion TEXT, activo BOOLEAN DEFAULT true
);
INSERT INTO dim_producto_clase (nombre) VALUES
    ('Producto Terminado'),('Materia Prima'),('Insumo'),
    ('Envase'),('Combo'),('Servicio'),('Activo Fijo')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_producto_subclase (
    id SERIAL PRIMARY KEY, clase_id INT NOT NULL REFERENCES dim_producto_clase(id),
    nombre VARCHAR(100) NOT NULL, activo BOOLEAN DEFAULT true
);
INSERT INTO dim_producto_subclase (clase_id, nombre) VALUES
    (1,'Terminado de Venta'),(1,'Terminado para Promoción'),
    (2,'MP Nacional'),(2,'MP Importada'),
    (3,'Insumo Directo'),(3,'Insumo Indirecto'),
    (4,'Envase Primario'),(4,'Envase Secundario'),
    (5,'Pack Promocional'),(5,'Combo Temporada')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_tipo_venta (
    id SERIAL PRIMARY KEY, nombre VARCHAR(100) NOT NULL, activo BOOLEAN DEFAULT true
);
INSERT INTO dim_tipo_venta (nombre) VALUES
    ('Venta Directa'),('Preventa'),('Venta por Mayor'),
    ('Venta por Menor'),('Venta Itinerante'),('Venta Online')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_tipo_compra (
    id SERIAL PRIMARY KEY, nombre VARCHAR(100) NOT NULL, activo BOOLEAN DEFAULT true
);
INSERT INTO dim_tipo_compra (nombre) VALUES
    ('Compra Nacional'),('Importación'),('Compra Local'),
    ('Servicio'),('Activo Fijo'),('Gasto Operativo')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_documento_tipo (
    id SERIAL PRIMARY KEY, codigo_sunat VARCHAR(2) UNIQUE NOT NULL,
    nombre VARCHAR(100) NOT NULL,
    simbolo_id INT NOT NULL REFERENCES dim_documento_simbolo(id),
    requiere_ruc BOOLEAN DEFAULT true, activo BOOLEAN DEFAULT true
);
INSERT INTO dim_documento_tipo (codigo_sunat, nombre, simbolo_id, requiere_ruc) VALUES
    ('01','FACTURA',1,true),('03','BOLETA DE VENTA',2,false),
    ('07','NOTA DE CRÉDITO',3,true),('08','NOTA DE DÉBITO',4,true),
    ('09','GUÍA DE REMISIÓN REMITENTE',5,true),
    ('20','COMPROBANTE DE RETENCIÓN',7,true),
    ('31','GUÍA DE REMISIÓN TRANSPORTISTA',8,true),
    ('40','COMPROBANTE DE PERCEPCIÓN',9,true)
ON CONFLICT (codigo_sunat) DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_tipo_cambio (
    id SERIAL PRIMARY KEY, moneda_origen_id INT NOT NULL REFERENCES dim_moneda(id),
    moneda_destino_id INT NOT NULL REFERENCES dim_moneda(id),
    valor DECIMAL(10,4) NOT NULL, fecha DATE NOT NULL,
    UNIQUE (moneda_origen_id, moneda_destino_id, fecha)
);
INSERT INTO dim_tipo_cambio (moneda_origen_id, moneda_destino_id, valor, fecha) VALUES
    (1,1,1.0000,CURRENT_DATE),(1,2,0.2700,CURRENT_DATE),
    (2,1,3.7200,CURRENT_DATE),(2,2,1.0000,CURRENT_DATE)
ON CONFLICT (moneda_origen_id, moneda_destino_id, fecha) DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_lista_precios (
    id SERIAL PRIMARY KEY, moneda_id INT NOT NULL REFERENCES dim_moneda(id),
    nombre VARCHAR(100) NOT NULL, activa BOOLEAN DEFAULT true
);
INSERT INTO dim_lista_precios (nombre, moneda_id) VALUES
    ('Lista General PEN',1),('Lista General USD',2),
    ('Lista Mayorista PEN',1),('Lista Promocional PEN',1)
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_estado_civil (
    id SERIAL PRIMARY KEY, nombre VARCHAR(50)
);
INSERT INTO dim_estado_civil (nombre) VALUES
    ('Soltero'),('Casado'),('Divorciado'),('Viudo'),('Conviviente')
ON CONFLICT DO NOTHING;

CREATE TABLE IF NOT EXISTS dim_sexo (
    id SERIAL PRIMARY KEY, nombre VARCHAR(20)
);
INSERT INTO dim_sexo (nombre) VALUES
    ('Masculino'),('Femenino')
ON CONFLICT DO NOTHING;
SQL;
    ...
    {
        $tables = [
            'dim_sexo', 'dim_estado_civil', 'dim_lista_precios', 'dim_tipo_cambio',
            'dim_documento_tipo', 'dim_tipo_compra', 'dim_tipo_venta',
            'dim_producto_subclase', 'dim_producto_clase', 'dim_clase_sunat',
            'dim_familia_sunat', 'dim_tipo_cliente', 'dim_segmento_sunat',
            'dim_nota_credito_tipo', 'dim_tipo_calculo_isc', 'dim_tipo_afeccion_igv',
            'dim_ubigeo', 'dim_provincia', 'dim_departamento',
            'dim_documento_simbolo', 'dim_dia_semana', 'dim_pais',
            'dim_unidad_medida', 'dim_moneda',
        ];
